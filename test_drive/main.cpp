...
int main(int argc, char *argv[]) {
  ...
  QDBusInterface interface_time(TIME_NAME, "/", TIME_NAME,
                                QDBusConnection::sessionBus());

  if (!interface_time.isValid()) {
    qDebug() << "test: Не удалось создать интерфейс для com.system.timeservice:"
             << interface_time.lastError().message();
    return -1;
  }
  ...
  if (reply1.isValid()) {
    ...
    qDebug() << "Ошибка при вызове метода GetSystemTime:"
             << reply1.error().message();
  }

  // подрубаем интерфес сервиса разрешений
  QDBusInterface interface_perm(PERM_NAME, "/", PERM_NAME,
                                QDBusConnection::sessionBus());

  if (!interface_time.isValid()) {
    qDebug() << "test: Не удалось создать интерфейс для "
                "com.system.permissionsservicetimeservice:"
             << interface_time.lastError().message();
    return -1;
  }
  ...
  if (reply2.isValid()) {
    ...
    qDebug() << "Ошибка при вызове метода RequestPermission:"
