...
PermissionsService::PermissionsService(QObject *parent)
  ...
  if (!this->db.open()) {
    ...
    this->db.setDatabaseName(
        "/home/sandor/prog/test_case_omp/test_case_omp_db.db");
...
bool PermissionsService::CheckApplicationHasPermission(
    const QString &applicationExecPath, int permissionEnumCode) {
...
QString PermissionsService::GetExecPath(const QString &client_path) {
  QDBusReply<uint> reply = QDBusConnection::sessionBus().interface()->call(
      "GetConnectionUnixProcessID", client_path);
  if (!reply.isValid()) {
    qDebug() << "Не удалось получить PID для сервиса:"
             << reply.error().message();
    return QString();
  }
...
QSqlQuery *PermissionsService::executeQuery(const QString &queryString) {
  ...
  if (!query->prepare(queryString)) {
    qWarning() << "dbcon: Ошибка при подготовке запроса:"
  ...
  if (!query->exec()) {
    qWarning() << "dbcon: Ошибка при выполнении запроса:"
               << query->lastError().text();
    delete query;
    return nullptr;
  }
