...
TimeService::TimeService(QObject *parent) : QDBusAbstractAdaptor(parent) {
  // bus connection
...
quint64 TimeService::GetSystemTime() {
  ...
  QDBusInterface interface(PERM_NAME, "/", PERM_NAME,
                           QDBusConnection::sessionBus());

  if (!interface.isValid()) {
    qDebug()
        << "Не удалось создать интерфейс для com.system.permissionsservice:"
  ...
  if (reply1.isValid()) {
    ...
    qDebug() << "Ошибка при вызове метода GetExecPath:"
  ...
  QDBusReply<bool> reply2 =
      interface.call("CheckApplicationHasPermission", path, int(0));
  if (reply2.isValid()) {
    qDebug() << "Ответ от CheckApplicationHasPermission:" << reply2.value();
    if (reply2.value()) {
      res =
          QDateTime::currentDateTime().toString("yyyyMMddhhmmss").toULongLong();
    } else {
      qDebug() << "UnauthorizedAccess";
      res = 2;
    }
  } else {
    qDebug() << "Ошибка при вызове метода CheckApplicationHasPermission:"
